...
static int udp_match_socket(jlos_hash_node_t *node, void *args1)
{
    jlos_udp_socket_t *socket = container_of(node, jlos_udp_socket_t, hash_node);
    uint32_t *args = args1;
    if (socket->m_local_ip != args[2] || socket->m_local_port != (uint16_t)args[3]) {
        return -1;
    }
    ...
    if (socket->m_listening) {
        socket->m_remote_ip = args[0];
        socket->m_remote_port = (uint16_t)args[1];
        return 0;
    }
    return -1;
}
...
bool jlos_udp_provider_on_internet_protocol_received(jlos_udp_provider_t* self, uint32_t srcIP_BE, uint32_t dstIP_BE, uint8_t *internet_protocol_payload, uint32_t m_size)
{
    ...
    uint32_t args[] = {srcIP_BE, msg->m_src_port, dstIP_BE, msg->m_dst_port};
