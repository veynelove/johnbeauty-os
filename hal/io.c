...
{
    jlos_hal_io_sanity_check(self->portnumber, 1, HAL_IO_OWNER);
    HAL_TRACE_IO(JLOS_HAL_TRACE_OP_WR8, self->portnumber, val);
    ops_safe()->write_io8(self, val);
}
uint8_t jlos_io8_read(jlos_io8_t *self)
{
    jlos_hal_io_sanity_check(self->portnumber, 0, HAL_IO_OWNER);
    uint8_t v = ops_safe()->read_io8(self);
    HAL_TRACE_IO(JLOS_HAL_TRACE_OP_RD8, self->portnumber, v);
    return v;
}
...
{
    jlos_hal_io_sanity_check(self->portnumber, 1, HAL_IO_OWNER);
    HAL_TRACE_IO(JLOS_HAL_TRACE_OP_WR16, self->portnumber, val);
...
uint16_t jlos_io16_read(jlos_io16_t *self)
{
    ...
    HAL_TRACE_IO(JLOS_HAL_TRACE_OP_RD16, self->portnumber, v);
    return v;
}
...
{
    jlos_hal_io_sanity_check(self->portnumber, 1, HAL_IO_OWNER);
    HAL_TRACE_IO(JLOS_HAL_TRACE_OP_WR32, self->portnumber, val);
