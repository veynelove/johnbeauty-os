...
void jlos_irq_handler_init(jlos_irq_handler_t *self, jlos_irq_manager_t *mgr, uint8_t irq)
{
...
uint32_t jlos_irq_manager_do_handle(jlos_irq_manager_t *self, uint8_t irq, uint32_t esp)
{
    return jlos_interrupt_manager_do_handle_interrupt(self, irq, esp);
}

void jlos_irq_manager_register(jlos_irq_manager_t *self, uint8_t irq, jlos_irq_handler_t *h)
{
