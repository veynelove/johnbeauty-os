...
void jlos_mouse_driver_init(jlos_mouse_driver_t* self, jlos_interrupt_manager_t *manager, jlos_mouse_event_handler_t *handler)
{
    ...
    jlos_interrupt_handler_init(&self->base_handler, manager, 0x2C);
    self->base_handler.handle_interrupt = (jlos_interrupt_handler_func_t)jlos_mouse_driver_handle_interrupt;
    
    jlos_driver_init(&self->base_driver);
    self->base_driver.activate = (void (*)(jlos_driver_t*))jlos_mouse_driver_activate;
    
    jlos_interrupt_manager_register_handler(manager, 0x2C, &self->base_handler);
}
...
void jlos_mouse_driver_activate(jlos_mouse_driver_t* self)
{
    ...
    jlos_port8_bit_write(&self->m_commandport, 0xA8);
    jlos_port8_bit_write(&self->m_commandport, 0x20);
    uint8_t status = jlos_port8_bit_read(&self->m_dataport) | 2;
    jlos_port8_bit_write(&self->m_commandport, 0x60);
    jlos_port8_bit_write(&self->m_dataport, status);

    jlos_port8_bit_write(&self->m_commandport, 0xD4);
...
uint32_t jlos_mouse_driver_handle_interrupt(jlos_mouse_driver_t* self, uint32_t m_esp)
{
    ...
    uint8_t status = jlos_port8_bit_read(&mouse->m_commandport);
    if (!(status & 0x20)) {  // 检查 bit 5：鼠标数据可用
        return m_esp;
    }
    mouse->buffer[mouse->m_offset] = jlos_port8_bit_read(&mouse->m_dataport);
